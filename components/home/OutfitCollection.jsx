...
const SIZES = ["Custom", "S", "M", "L", "XL", "XXL"];

const FALLBACK_OUTFITS = [
  { name: "Royal Bandhgala Sherwani", price: 4999, image: "/royal-sherwani.webp" },
  { name: "Classic Jodhpuri Suit", price: 3499, image: "/jodhpuri-suit.webp" },
  { name: "Modern Indowestern", price: 3999, image: "/indowestern.webp" },
  { name: "Designer Kurta Pajama Set", price: 1199, image: "/kurta-pajama.webp" },
];

function OutfitCard({ outfit, delay }) {
  const { toggleWishlist, isInWishlist } = useWishlist();
  const { showToast } = useToast();
  const isWishlisted = isInWishlist(outfit.id || outfit.name);
  const href = outfit.slug ? `/shop/${outfit.slug}` : "/shop?type=outfit";

  const handleWishlistToggle = (e) => {
    e.preventDefault();
    e.stopPropagation();
    toggleWishlist({
      id: outfit.id || outfit.name,
      name: outfit.name,
      price: outfit.price,
      image: outfit.image,
      slug: outfit.slug || "shop",
      productType: "outfit",
      inStock: outfit.inStock !== undefined ? outfit.inStock : true,
    });
    showToast(
      isWishlisted
        ? `${outfit.name} removed from wishlist.`
        : `${outfit.name} added to wishlist.`
    );
  };
  ...
  return (
    <Reveal delay={delay}>
      <Link
        href={href}
        className={`group flex flex-col overflow-hidden rounded-2xl border bg-white transition-all duration-300 hover:-translate-y-1.5 hover:border-gold-400/40 hover:shadow-gold ${outfit.isFeatured ? "border-gold-400/50 shadow-gold" : "border-ink/10"
          }`}
      >
        ...
        <div className="flex flex-1 flex-col p-3.5 sm:p-4">
          <h3 className="font-display text-base font-medium text-ink sm:text-lg">{outfit.name}</h3>
          <p className="mt-1 text-base font-semibold text-gold-600 sm:text-lg">
            {outfit.price != null ? `₹${outfit.price.toLocaleString("en-IN")}` : "—"}
            <span className="text-xs font-normal text-ink/40 ml-1">Stitching Included</span>
          </p>
        </div>
      </Link>
    </Reveal>
...
export default function OutfitCollection({ outfits = [], eyebrow = "Bespoke Stitching", heading = "Our Custom Outfits" }) {
  const items = outfits.length > 0 ? outfits : FALLBACK_OUTFITS;
    ...
    <section className="relative bg-white py-16 sm:py-24">
      <div className="mx-auto max-w-wrap px-4 sm:px-6 md:px-12">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 sm:gap-6 lg:grid-cols-4">
          {items.map((outfit, i) => (
            <OutfitCard key={outfit.id || outfit.name} outfit={outfit} delay={i * 80} />
          ))}
        </div>
      </div>
    </section>
